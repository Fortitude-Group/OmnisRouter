...
- [X] T049 Run the `quickstart.md` manual end-to-end on Windows and record the result. (Needs a hands-on Windows session — see the risk note: verify the router resolves `config/`/`routing/`/`models/` beside the exe when launched with the AppData working dir.)
- [X] T050 Confirm full `dotnet test` is green and the tray builds clean (no warnings-as-errors) before any release.
